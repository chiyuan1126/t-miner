...
public class TableDataExtraction {
	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		for (File pf : files) {
			...
			for (int i = 0; i < 20; i++) {
				//jsonfile是二级目录
				String json_path = json_dir + File.separator + fid + File.separator + fid + "-" + i + ".json";
				...
				for (int k = 0; k < ja.size(); k++) {
					...
					String img_path = img_dir + File.separator + fid + File.separator + fid + "-" + i + ".jpg";
					//System.out.println("----------------------------->"+img_path);
					System.out.println(">>>>>>>>>>>>>>>Table:"+k+"\t"+img_path+"<<<<<<<<<<<<<<<");
					String updated_img_path = reimg_dir + File.separator  + fid + File.separator + fid + "-" + i + ".jpg";
	...
	private static void write2imgfile(String table_dir, File pf, int i,int k, float x_from, float x_to, float y_from,
		...
		String filePath = table_dir + File.separator + pf.getName() + File.separator + pf.getName() + "-"+i+"-"+k+".jpg";
		File output = new File(filePath);
		output.getParentFile().mkdirs();
		System.out.println("write to "+table_dir+pf.getName()+"-"+i+"-"+k+".jpg");
		Imgcodecs.imwrite(filePath, target);
	}
	...
	private static String nearestLine(String imgpath, int startrow, int endrow,float x_from,float x_to) {
		...
		//Imgcodecs.imwrite("D:\\test\\test\\111111.jpg", img);
